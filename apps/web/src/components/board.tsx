...
const pieceElements = (piece: TayloredPiece): ExcalidrawElement[] => {
  ...
  const elements = convertToExcalidrawElements([
    { id: `piece-image-${piece.id}`, type: "image", x, y, width: piece.position.width, height: piece.position.height, angle: piece.rotation ?? 0, scale: [piece.flipX ? -1 : 1, piece.flipY ? -1 : 1], crop: piece.crop ?? null, fileId: toFileId(piece.id, piece.imageVariant), customData: { pieceId: piece.id, elementKind: "image", layer: piece.imageZIndex ?? piece.zIndex ?? 0, product: piece.product } },
    ...(piece.showLabel === false ? [] : [{ id: `piece-label-${piece.id}`, type: "text" as const, x, y: y + piece.position.height + 14, text: [piece.product?.title, piece.product?.price].filter(Boolean).join(" · ") || "Saved product", fontSize: 18, strokeColor: "#28241f", customData: { pieceId: piece.id, elementKind: "label", layer: piece.labelZIndex ?? piece.zIndex ?? 0 } }]),
  ]);
...
export function Board() {
  ...
  const [selectedElementKind, setSelectedElementKind] = useState<"image" | "label">("image");
  ...
  const [captureHelpOpen, setCaptureHelpOpen] = useState(true);
  ...
  const elements = useMemo(() => boardPieces.flatMap(pieceElements).sort((a, b) => ((a.customData as { layer?: number } | undefined)?.layer ?? 0) - ((b.customData as { layer?: number } | undefined)?.layer ?? 0)), [boardPieces]);
  ...
  const duplicatePiece = async (source: TayloredPiece) => {
    const now = new Date().toISOString();
    const duplicate: TayloredPiece = { ...source, id: crypto.randomUUID(), createdAt: now, updatedAt: now, onBoard: true, isBoardDuplicate: true, showLabel: false, position: { ...source.position, x: source.position.x + 36, y: source.position.y + 36 }, imageZIndex: Math.max(0, ...boardPieces.map((piece) => piece.imageZIndex ?? piece.zIndex ?? 0)) + 1 };
  ...
  return <main className="flex min-h-screen flex-col overflow-x-hidden bg-[#eeebe4] text-[#24221f] lg:h-screen lg:overflow-hidden">
    ...
    <div className="ts-layout flex min-h-0 flex-1 flex-col lg:flex-row">
      <aside className="flex w-full shrink-0 flex-col border-b border-black/[.07] bg-[#f8f6f1] lg:w-[298px] lg:border-b-0 lg:border-r">
        ...
        {captureHelpOpen && <div className="mt-auto border-t border-black/[.07] p-5"><div className="relative rounded-2xl bg-[#26392f] p-4 text-white"><button onClick={() => setCaptureHelpOpen(false)} aria-label="Close capture help" title="Close" className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full text-white/65 hover:bg-white/10 hover:text-white"><X className="h-4 w-4"/></button><div className="flex items-center gap-2 pr-7 text-xs font-semibold"><WandSparkles className="h-4 w-4 text-[#dec8a5]"/>Capture from anywhere</div><p className="mt-2 text-[11px] leading-5 text-white/60">Install the Chrome extension, save a product while you shop, and it appears on this board.</p><button onClick={() => { setExtensionHelpOpen(true); setTourOpen(false); }} className="mt-3 flex items-center gap-2 text-[11px] font-semibold text-[#dec8a5]"><Link2 className="h-3.5 w-3.5"/>Extension setup</button></div></div>}
      </aside>
      ...
      <section className="relative h-[68vh] min-h-[520px] min-w-0 flex-1 bg-[#dedbd3] p-3 sm:p-4 lg:h-auto lg:min-h-0">
        ...
        {selectedPiece && selectedPiece.onBoard !== false && selectedElementKind === "image" && <div className="absolute left-1/2 top-5 z-30 flex max-w-[calc(100%-32px)] -translate-x-1/2 items-center gap-1 overflow-x-auto rounded-xl border border-black/[.09] bg-white/95 p-1.5 shadow-[0_10px_32px_rgba(42,37,31,.16)] backdrop-blur"><button onClick={copySelectedPiece} title="Copy selected image" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><Copy className="h-4 w-4"/></button><button disabled={!copiedPieceId} onClick={pasteCopiedPiece} title="Paste another image instance" className="rounded-lg px-2 py-1.5 text-[11px] font-semibold hover:bg-black/[.05] disabled:opacity-30">Paste</button><button onClick={() => void duplicatePiece(selectedPiece)} title="Duplicate selected image" className="rounded-lg px-2 py-1.5 text-[11px] font-semibold hover:bg-black/[.05]">Duplicate</button><span className="mx-1 h-5 w-px bg-black/10"/><button onClick={() => void updateBoardAppearance({ rotation: (selectedPiece.rotation ?? 0) + Math.PI / 2 })} title="Rotate 90 degrees" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><RotateCw className="h-4 w-4"/></button><button onClick={() => void updateBoardAppearance({ flipX: !selectedPiece.flipX })} title="Flip horizontally" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><FlipHorizontal className="h-4 w-4"/></button><button onClick={startCrop} title="Crop (or double-click the image)" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><Crop className="h-4 w-4"/></button><span className="mx-1 h-5 w-px bg-black/10"/><button onClick={() => void updateBoardAppearance({ imageZIndex: Math.min(...boardPieces.map((piece) => piece.imageZIndex ?? piece.zIndex ?? 0)) - 1 })} title="Send selected image to back" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><SendToBack className="h-4 w-4"/></button><button onClick={() => void updateBoardAppearance({ imageZIndex: Math.max(...boardPieces.map((piece) => piece.imageZIndex ?? piece.zIndex ?? 0)) + 1 })} title="Bring selected image to front" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg hover:bg-black/[.05]"><BringToFront className="h-4 w-4"/></button></div>}
