...
import { ArrowRight, Armchair, Check, CircleDollarSign, ExternalLink, Grid2X2, ImageIcon, ImagePlus, Layers3, Link2, PackageOpen, Plus, RefreshCw, Scissors, Search, Trash2, WandSparkles, X } from "lucide-react";
...
export function Board() {
  ...
  const addPieces = useCallback(async (incoming: TayloredPiece[]) => {
    ...
    for (const piece of fresh) {
      if (!piece.imageDataUrl || piece.type !== "product") continue;
      try {
  ...
  const sharePreview = async () => {
    try {
      ...
      setStatus("Board link copied — paste it anywhere");
  ...
  return <main className="flex h-screen min-w-[1040px] flex-col overflow-hidden bg-[#eeebe4] text-[#24221f]">
    ...
    <header className="flex h-[72px] shrink-0 items-center border-b border-black/[.07] bg-[#f8f6f1] px-5">
      <div className="flex w-[278px] items-center gap-3 border-r border-black/[.07]"><div className="relative h-11 w-14 overflow-hidden"><Image src="/tayloredspace-logo-v2.png" alt="TayloredSpace TS swallow" fill sizes="56px" priority className="object-contain"/></div><div><h1 className="font-serif text-[21px] leading-none tracking-tight">TayloredSpace</h1><p className="mt-1.5 text-[10px] font-semibold uppercase tracking-[.17em] text-[#9a6b58]">Design what feels like you</p></div></div>
      <div className="flex flex-1 items-center justify-between pl-6"><div className="text-sm font-semibold">Austin living room</div><div className="flex items-center gap-3"><div className="flex max-w-[310px] items-center gap-2 truncate rounded-full bg-[#e9efe9] px-3 py-2 text-[11px] font-medium text-[#4b6354]"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#66866f]"/>{status}</div><button onClick={() => { setTourStep(0); setTourOpen(true); }} className="flex items-center gap-2 rounded-full border border-[#9a6b58]/20 bg-[#f4ece4] px-4 py-2 text-xs font-semibold text-[#795747]"><WandSparkles className="h-3.5 w-3.5"/>Demo tour</button><button onClick={() => void sharePreview()} className="rounded-full border border-black/10 bg-white px-4 py-2 text-xs font-semibold shadow-sm">Share board</button></div></div>
    </header>
